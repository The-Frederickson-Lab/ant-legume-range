...
pgls2 <- gls(log(total_area_introduced/1e+6 + 1) ~  EFN+Domatia+fixer+
            total_area_native + abs_lat_native + uses_num_uses+nathis, 
...
pgls3 <- gls(log((total_area_native/1e+6) + 1) ~ EFN*Domatia + fixer+ abs_lat_native+ nathis + uses_num_uses, correlation = corPagel(1, phy = pruned.tree.pgls, form = ~ Phy2), method = "ML", data = range_pgls)
summary(pgls3)
...
binom <- glm(introducedY~woody, data=df, family="binomial")
summary(binom)

binomial1 <- glmer(introducedY~Domatia+EFN+fixer+scale(total_area_native)+woody+annual+scale(abs_lat_native)+uses_num_uses+(1|tribe_ncbi), data=df, family="binomial")
summary(binomial1) 
Anova(binomial1, type=3)
plot(binomial1)

#Fit binomial model without random effect
binomial2 <- glm(introducedY~EFN+Domatia+fixer+scale(total_area_native)+woody+annual+scale(abs_lat_native)+uses_num_uses, data=df, family="binomial")
summary(binomial2) 
...
binomial3 <- glmer(introducedY~AM*EM+scale(total_area_native)+annual+woody+scale(abs_lat_native)+uses_num_uses+(1|tribe_ncbi), data=subset(df, !is.na(myco)), family="binomial", control=glmerControl(optimizer="nloptwrap", optCtrl=list(maxfun=2e6)))
summary(binomial3) 
Anova(binomial3, type=3)
plot(binomial3)

#Number of introduced ranges
lmer4 <- lmer(log(num_introduced)~AM+EM+scale(total_area_native)+annual+woody+scale(abs_lat_native)+uses_num_uses+(1|tribe_ncbi), data=subset(legume_range_df_introducedY, !is.na(myco)))
summary(lmer4)
Anova(lmer4, type=3)
plot(lmer4)
...
#add a column for ant subfamily
genus <- word(area$Phy, 1, 1, sep="_") #creating the vector of genera for all species
subfamily <- c()
...
summary.ant.efn1 <- subset(area, introducedY == 1 & !is.na(EFN)) %>% group_by(EFN) %>% summarize(n=n(), mean_introduced = mean(Non_cont, na.rm=TRUE), sd_introduced = sd(Non_cont, na.rm=TRUE), se_introduced = sd_introduced/sqrt(n))
...
summary.ant.dom1 <- subset(area, introducedY == 1 & !is.na(Domatia)) %>% group_by(Domatia) %>% summarize(n=n(), mean_introduced = mean(Non_cont, na.rm=TRUE), sd_introduced = sd(Non_cont, na.rm=TRUE), se_introduced = sd_introduced/sqrt(n))
...
summary.ant.elaiosome1 <- subset(area, introducedY == 1 & !is.na(Seed_Dispersal)) %>% group_by(Seed_Dispersal) %>% summarize(n=n(), mean_introduced = mean(Non_cont, na.rm=TRUE), sd_introduced = sd(Non_cont, na.rm=TRUE), se_introduced = sd_introduced/sqrt(n))
...
area_phy <- area[area$Phy %in% phy_int, c("Phy", "abs_lat_native", "total.area.introduced", "total.area.native", "Non_cont",  "EFN", "Domatia", "Seed_Dispersal")] #dataset for pgls
...
a3 <- gls(log(Non_cont+1) ~  EFN + Domatia + Seed_Dispersal +  total.area.native + abs_lat_native,  correlation = corPagel(1, phy = pruned_ant_tree, form = ~Phy), method = "ML", data = area_phy)  
summary(a3)
