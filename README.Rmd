...
```{r Legume plots for number of introduced ranges, warning=FALSE, message=FALSE, fig.width=8, fig.height=6, dpi=300}
...
```{r Legume Pagel, warning=FALSE, message=FALSE}
...
```
#### Mixed models
Another possible approach is to use mixed models with legume tribe as a random effect to account for the non-independence of species in a tribe. The response variables (number of introduced ranges and total introduced area) are very non-normal so we fit two models: a binomial GLMM modelling whether or not a legume species is introduced, and linear mixed models of the number of introduced ranges for introduced species only. 
...
```{r ant Pagel, warning=FALSE, message=FALSE}
