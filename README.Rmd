...
pgls1 <- gls(log(num_introduced + 1) ~  factor(EFN, contrasts = list(EFN = contr.treatment))+
             ...
             +total_area_native + abs_lat_native + uses_num_uses + woody + annual, correlation = corAR1(phy = pruned.tree.pgls, form = ~ Phy2, method = "ML", data = range_pgls)) 
summary(pgls1)

## Two issues - something wrong with the log function when using range sizes, and something with
##the categorical variables being used

#correlation = corAR1(form = ~1|Phy2), method = "ML", data = range_pgls)
...
model <- gls(y ~ x1 + factor(x2, contrasts = list(x2 = contr.treatment)) + factor(x3, contrasts = list(x3 = contr.treatment)) + x4, data = mydata)

pgls1 <- gls(log(num_introduced + 1) ~ woody , correlation = corPagel(1, phy = pruned.tree.pgls, form = ~ Phy2), method = "ML", data = range_pgls) 
summary(pgls1)
...
pgls2 <- gls(log(total_area_introduced/1e+6 + 1) ~  EFN, 
            correlation = corPagel(1, phy = pruned.tree.pgls, form = ~ Phy2), 
            method = "ML", data = range_pgls) 
...
#removed the interaction when nonsig or the number of species showing trait combinations was
#too small

#Domatia interaction significant
...
#add a column for ant subfamily - no run rquired
...
### dplyr::summarize ant dataset

```{r}

area <- read.csv("ant_areas.csv")
#area <- merge(area, ncontig, by = "Phy")
#area <- area[, -10] 
...
summary.ant.efn1 <- subset(area, introducedY == 1 & !is.na(EFN)) %>% group_by(EFN) %>% dplyr::summarize(n=n(), mean_introduced = mean(Non_cont, na.rm=TRUE), sd_introduced = sd(Non_cont, na.rm=TRUE), se_introduced = sd_introduced/sqrt(n))
...
summary.ant.dom1 <- subset(area, introducedY == 1 & !is.na(Domatia)) %>% group_by(Domatia) %>% dplyr::summarize(n=n(), mean_introduced = mean(Non_cont, na.rm=TRUE), sd_introduced = sd(Non_cont, na.rm=TRUE), se_introduced = sd_introduced/sqrt(n))
...
summary.ant.elaiosome1 <- subset(area, introducedY == 1 & !is.na(Seed_Dispersal)) %>% group_by(Seed_Dispersal) %>% dplyr::summarize(n=n(), mean_introduced = mean(Non_cont, na.rm=TRUE), sd_introduced = sd(Non_cont, na.rm=TRUE), se_introduced = sd_introduced/sqrt(n))
...
lmer11 <- lmer(log(total.area.introduced)~num.mm + scale(abs_lat_native)+scale(total.area.native)+(1|Tribe), data=subset(area, introducedY == 1))
summary(lmer11)
Anova(lmer11, type=3)
